...
|
  (?P<id>ID:[ ]?([0-9A-Za-z/.+]{8,14})(?=[^0-9A-Za-z/.+]|$)) # ID（8,10桁 +PC/携帯識別フラグ）
)
}x';
	...
	//ADDEDNEW
	$GLOBALS['ninkiresu_array'] = array();
	$begin_thread = $is_fragment ? '' : "<div class=\"thread\">\n";
	...
	//ADDEDNEW
	$ninkilinks_str = '';
	foreach ($GLOBALS['ninkiresu_array'] as $resnum) {
		$ninkilinks_str .= '<a href=#r' . $resnum . '>'. $resnum . '</a> ';
	}
	$begin_thread .= '人気レス一覧<br>' . $ninkilinks_str . '<p>';

        return $begin_thread . $buf['body'] . $buf['q'];
    ...
    protected function _quotebackHorizontalListHtml($anchors, $resnum)
    {
        ...
        foreach ($anchors as $idx => $anchor) {
            ...
            $ret .= sprintf('<div>【参照レス %s:%s】</div>',$count,$anchor_link);
            $ret .= '</div>';
            $count++;
        }
	...
	if ($count >= 3){
		$exists = 0;
		foreach ($GLOBALS['ninkiresu_array'] as $num) {
			if ($num == $resnum){$exists=1;break;}
		}
		if ($exists==0){
			array_push($GLOBALS['ninkiresu_array'], $resnum);
		}
		$ret .= '<a href=#header>[戻る]</a> ';
	}
